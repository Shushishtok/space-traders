...
import { AgentModel, ExtractionModel, ShipModel, SurveyModel, TransactionModel, WaypointModel } from "../sequelize/models";
...
export async function getShips(shipSymbols: string[]) {
...
export async function navigateShips(shipSymbols: string[], waypointSymbol: string) {
	const shipsApi = getFleetApi();
...
export async function dockShips(shipSymbols: string[]) {
	const shipsApi = getFleetApi();
...
export async function refuelShips(shipSymbols: string[]) {
	const shipsApi = getFleetApi();
...
export async function orbitShips(shipSymbols: string[]) {
	const shipsApi = getFleetApi();
...
export async function extractResources(shipSymbols: string[], survey?: Survey) {
	const shipsApi = getFleetApi();
...
export async function sellCargo(shipSymbols: string[], sellObject: SellCargoRequest) {
	const shipsApi = getFleetApi();	
...
export async function purchaseCargo(shipSymbols: string[], purchaseObject: PurchaseCargoRequest) {
	const shipsApi = getFleetApi();	
...
export async function createSurvey(shipSymbols: string[]) {
	const shipsApi = getFleetApi();
...
export async function warpShips(shipSymbols: string[], waypointSymbol: string) {
	const shipsApi = getFleetApi();
...
export async function jumpShips(shipSymbols: string[], systemSymbol: string) {
	const shipsApi = getFleetApi();
...
export async function setFlightMode(shipSymbols: string[], flightMode: ShipNavFlightMode) {
	const shipsApi = getFleetApi();
...
export async function installMount(shipSymbols: string[], mountSymbol: string) {
	const shipsApi = getFleetApi();
	...
	const promises = shipSymbols.map(async shipSymbol => {
		...
		await Promise.all([updateAgentPromise, updateShipPromise]);
...
export async function updateRoles(shipSymbols: string[], addRoles: ShipActionRole[] = [], removeRoles: ShipActionRole[] = []) {
...
export async function createChart(shipSymbol: string) {
	const shipsApi = getFleetApi();
	
	const data = await tryApiRequest(async () => {
		const result = await shipsApi.createChart(shipSymbol);
		const { data } = result;
		return data;
	}, "Could not set mount");
	...
	Logger.info(`Ship with symbol ${shipSymbol} successfully charted waypoint ${data.data.waypoint.symbol}. Its traits are: ${JSON.stringify(data.data.waypoint.traits)}`);
	
	await WaypointModel.update({ ...data.data.waypoint }, { where: { symbol: data.data.waypoint.symbol } });
